...
const Lambaikiemtra = () => {
  ...
  const getlichsu = async () => {
    try {
      ...
      const { result, code } = await laylichsu(test_id, userId());
      if (code === 1000) {
  ...
  const getcauhoi = async () => {
    try {
      ...
      const { result, code } = await laycauhoi(test_id);
      if (code === 1000) {
  ...
  const handleSubmit = async () => {
    ...
    const submittedAnswers = currentQuestion.map((q) => ({
      ...
      answers: answers[q.questions_id] || "Chưa trả lời", // Câu trả lời được chọn
      is_correct: answers[q.questions_id] === q.correct_answers[0], // So sánh đúng/sai
      user_id: userId(),
      test_id: test_id,
    }));
    ...
    const totalScore = submittedAnswers.reduce((score, item) => {
      return item.is_correct ? score + pointsPerQuestion : score;
    }, 0);
              ...
              <div className="space-y-2">
                {Object.keys(history).map((attemptNumber) => (
                  <button
                    ...
                    className={`w-full py-2 rounded border ${selectedHistory === attemptNumber
                ...
                <div>
                  ...
                  {history[selectedHistory].entries.map((entry, index) => {
                    const question = currentQuestion.find(
                      (q) => q.questions_id === entry.question_id
                    );
                    const isCorrect = entry.answers === (question?.correct_answers[0] || "");
                    return (
                ...
                {q.options.map((option, idx) => (
                  <label key={idx} className="block">
                    <input
                      ...
                      value={option}
                      onChange={() => handleAnswerChange(q.questions_id, option)}
                      className="mr-2"
                    />
                    {option}
                  </label>
        ...
        <div className="grid grid-cols-5 gap-2">
          {currentQuestion.map((q, index) => (
            <button
              ...
              className={`w-12 h-12 flex items-center justify-center rounded border ${answers[q.questions_id] ? "bg-green-500 text-white" : "bg-gray-200"
                }`}
            >
              {index + 1}
            </button>
