...
import { getMyClass, sualop,xoalop } from '../services/ClassService';
import { laybaikiem } from "../services/TestService";
...
const Trangchu = () => {
	...
	const getClassByUser1ID = async () => {
		try {
			const { result, code } = await getMyClass()
			if (code === 1000) {
							...
							<div className="grid grid-cols-4 gap-4">
								{lophoc.map((card, index) => (
									<Card key={index} {...card}
										...
										onDelete={() => handleDeleteClass(card.classid)} // Truyền hàm xóa
									/>
