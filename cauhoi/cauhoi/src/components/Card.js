...
const Card = ({ name, soluongthanhvien, imageclass, classid,role ,onEdit, onDelete}) => {
  ...
  return (
	...
	<Link to={`/baikiemtra?classid=${classid}&role=${role}`}>
	  <img src={imageclass} alt={name} className="w-full h-48 object-cover" />
	</Link>
	<div className="p-4">
	  <Link to={`/baikiemtra?classid=${classid}&role=${role}`}>
